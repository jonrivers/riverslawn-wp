 ...
 * Rivers Lawn Theme
 *
 * This file adds the page header to the Rivers Lawn Theme.
 *
 * @package   StudioPro
...
add_filter( 'genesis_attr_entry', 'riverslawn_entry_attr' );
/**
 ...
 * @return array
 */
function riverslawn_entry_attr( $atts ) {
...
add_action( 'genesis_before', 'riverslawn_page_header_setup' );
/**
 * Set up page header.
 *
 ...
 * @return void
 */
function riverslawn_page_header_setup() {
	...
	add_action( 'riverslawn_page_header', 'genesis_do_posts_page_heading' );
	add_action( 'riverslawn_page_header', 'genesis_do_date_archive_title' );
	add_action( 'riverslawn_page_header', 'genesis_do_taxonomy_title_description' );
	add_action( 'riverslawn_page_header', 'genesis_do_author_title_description' );
	add_action( 'riverslawn_page_header', 'genesis_do_cpt_archive_title_description' );
...
add_action( 'genesis_before_content_sidebar_wrap', 'riverslawn_remove_404_title' );
/**
 * Remove default 404 entry title.
 *
 * These filters are placed in a separate function to the rest of the page
 * header setup filters because it needs to be called at a later hook,
 * otherwise it removes the custom title in the page header section.
 *
 * @since  2.2.0
 *
 * @return void
 */
function riverslawn_remove_404_title() {
...
add_action( 'riverslawn_page_header', 'riverslawn_page_title', 10 );
/**
 ...
 * @return void
 */
function riverslawn_page_title() {
	...
	if ( class_exists( 'WooCommerce' ) && is_shop() ) {
		...
		genesis_markup( array(
			'open'    => '<h1 %s>',
			'close'   => '</h1>',
			'content' => apply_filters( 'riverslawn_latest_posts_title', __( 'Latest Posts', 'rivers-lawn' ) ),
			'context' => 'archive-title',
		) );
		...
		genesis_markup( array(
			'open'    => '<h1 %s>',
			'close'   => '</h1>',
			'content' => apply_filters( 'genesis_404_entry_title', __( 'Not found, error 404', 'rivers-lawn' ) ),
			'context' => 'archive-title',
		) );
		...
		genesis_markup( array(
			'open'    => '<h1 %s>',
			'close'   => '</h1>',
			'content' => apply_filters( 'genesis_search_title_text', __( 'Search results for: ', 'rivers-lawn' ) ) . get_search_query(),
			'context' => 'archive-title',
		) );
...
add_action( 'riverslawn_page_header', 'riverslawn_page_excerpt', 20 );
/**
 ...
 * @return void
 */
function riverslawn_page_excerpt() {
...
add_action( 'genesis_before_content_sidebar_wrap', 'riverslawn_page_header' );
/**
 * Display the page header.
 *
 * Conditionally outputs the opening and closing page header markup and runs
 * riverslawn_page_header which all of our header functions are hooked to.
 *
 * @since  2.2.4
 *
 * @return void
 */
function riverslawn_page_header() {

	// Output opening markup.
	if ( apply_filters( 'riverslawn_page_header_open', true ) ) {
	 ...
	 * Do page header hook.
	 *
	 * @hooked riverslawn_page_title - 10
	 * @hooked riverslawn_page_excerpt - 20
	 ...
	 */
	do_action( 'riverslawn_page_header' );

	// Output closing markup.
	if ( apply_filters( 'riverslawn_page_header_close', true ) ) {
