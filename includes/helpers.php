 ...
 * Rivers Lawn Theme
 *
 * This file adds helper functions used in the Rivers Lawn Theme.
 *
 * @package   StudioPro
 ...
 * @return string
 */
function riverslawn_sanitize_number( $number, $setting ) {
 ...
 * values. Used in riverslawn_customizer_output to handle transparency.
 *
 * @since  0.1.0
 *
 ...
 * @return string
 */
function riverslawn_hex_to_rgba( $color, $opacity ) {
 ...
 * converts it into and RGBA using riverslawn_hex_to_rgba if necessary.
 *
 * @since  0.1.0
 *
 ...
 * @return string
 */
function riverslawn_rgba_to_hex( $string ) {
	...
	if ( strpos( $string, ',' ) != true ) {
		...
		$string = 'rgba(' . riverslawn_hex_to_rgba( $string, '1' ) . ')';
 ...
 */
function riverslawn_minify_css( $css ) {
 ...
 * @return bool
 */
function riverslawn_is_woocommerce_page() {
...
add_filter( 'display_posts_shortcode_post_class', 'riverslawn_dps_column_classes', 10, 4 );
/**
 * Column Classes
 *
 ...
 */
function riverslawn_dps_column_classes( $classes, $post, $listing, $atts ) {
 ...
 * @return string
 */
function riverslawn_custom_header() {
