 ...
 * Rivers Lawn Theme
 *
 * This file adds customizer settings to the Rivers Lawn theme.
 *
 * @package   StudioPro
 ...
 */
$riverslawn_colors = array(
	'gradient_left'  => 'rgba(100,66,255,0.9)',
	'gradient_right' => 'rgba(12,180,206,0.9)',
);

add_action( 'customize_register', 'riverslawn_customize_register' );
/**
 ...
 * @return void
 */
function riverslawn_customize_register( $wp_customize ) {

	// Globals.
	global $wp_customize, $riverslawn_colors;
	...
	$wp_customize->add_setting(
		'riverslawn_logo_size',
		array(
			'capability'        => 'edit_theme_options',
			'default'           => 100,
			'sanitize_callback' => 'riverslawn_sanitize_number',
		)
	);
	...
	$wp_customize->add_control( new WP_Customize_Control(
		...
		'riverslawn_logo_size',
		array(
			'label'       => __( 'Logo Size', 'rivers-lawn' ),
			'description' => __( 'Set the logo size in pixels. Default is 100.', 'rivers-lawn' ),
			'settings'    => 'riverslawn_logo_size',
	...
	$wp_customize->add_setting( 'riverslawn_sticky_header',
		array(
			'capability' => 'edit_theme_options',
			'default'    => false,
		)
	...
	$wp_customize->add_control( new WP_Customize_Control(
		$wp_customize,
		'riverslawn_sticky_header',
		array(
			'label'    => __( 'Enable sticky header', 'rivers-lawn' ),
			'settings' => 'riverslawn_sticky_header',
	...
	// Add header settings.
	$wp_customize->add_setting( 'riverslawn_blog_layout',
		array(
			'capability' => 'edit_theme_options',
			'default'    => 'masonry',
		)
	...
	$wp_customize->add_control( new WP_Customize_Control(
		$wp_customize,
		'riverslawn_blog_layout',
		array(
			'label'    => __( 'Blog Layout', 'rivers-lawn' ),
			'settings' => 'riverslawn_blog_layout',
			'section'  => 'genesis_layout',
			'type'     => 'select',
			'choices'  => array(
				'default' => 'Default',
	 ...
	 * the $riverslawn_colors` array at the start of this file.
	 */
	foreach ( $riverslawn_colors as $id => $rgba ) {

		// Format ID and label.
		$setting = "riverslawn_{$id}_color";
		$label   = ucwords( str_replace( '_', ' ', $id ) ) . __( ' Color', 'rivers-lawn' );
...
add_action( 'wp_enqueue_scripts', 'riverslawn_customizer_output', 100 );
/**
 ...
 *
 * @var   array $riverslawn_colors Global theme colors.
 */
function riverslawn_customizer_output() {

	// Defined at the top of this file.
	global $riverslawn_colors;

	// Other customizer settings.
	$logo_size = get_theme_mod( 'riverslawn_logo_size', 100 );
	 ...
	 * declare the colors once in the `$riverslawn_colors` array,
	 * and they can be used in multiple ways.
	 */
	foreach ( $riverslawn_colors as $id => $hex ) {

		${"$id"} = get_theme_mod( "riverslawn_{$id}_color", $hex );
	...
	$accent = new Studio_Color( riverslawn_rgba_to_hex( $gradient_left ) );
	$mix    = '#' . $accent->mix( riverslawn_rgba_to_hex( $gradient_right ) );
	 ...
	 */
	$css .= ( $riverslawn_colors['gradient_left'] !== $gradient_left || $riverslawn_colors['gradient_right'] !== $gradient_right ) ? "
	...
	if ( class_exists( 'WooCommerce') && riverslawn_is_woocommerce_page() ) {
	...
	if ( ! empty( $css ) ) {
		...
		wp_add_inline_style( $handle, riverslawn_minify_css( $css ) );
